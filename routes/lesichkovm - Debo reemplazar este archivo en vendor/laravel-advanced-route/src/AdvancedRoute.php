...
class AdvancedRoute {
    ...
    public static function controller($path, $controllerClassName) {
        ...
        foreach ($publicMethods as $method) {
            ...
            if (self::stringStartsWith($methodName, 'put')) {
                $slug = self::slug($method);
                //var_dump($slug);
                Route::put($path . '/' . $slug, $controllerClassName . '@' . $methodName);
            }
            if (self::stringStartsWith($methodName, 'delete')) {
                $slug = self::slug($method);
                //var_dump($slug);
                Route::delete($path . '/' . $slug, $controllerClassName . '@' . $methodName);
            }
        }
    ...
    protected static function slug($method) {
        $methodName = $method->name;
        $cleaned = str_replace(['any', 'put', 'get', 'post', 'delete', 'put'], '', $methodName);
